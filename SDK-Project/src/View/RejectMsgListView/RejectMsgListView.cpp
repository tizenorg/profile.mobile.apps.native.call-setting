...
namespace CallSettings { namespace View {
	...
	{
		SelectableItem *msgItem = m_pGenlist->appendItem<SelectableItem>(nullptr, msgText.c_str(), false, isTranslatable);
		RETVM_IF(!msgItem, false, "Failed to append item");

		if (m_isSelectModeEnabled) {
			msgItem->enableSelectMode();
		}
		...
		msgItem->setSelectHandler(SelectableItem::ItemSelectHandler::wrap<RejectMsgListView, &RejectMsgListView::onMsgItemSelected>(this));
	...
	void RejectMsgListView::onMsgItemSelected(SelectableItem *item)
	{
		auto it = std::find_if(m_msgItemsArray.begin(), m_msgItemsArray.end(),
		...
		RETM_IF(it == m_msgItemsArray.end(), "Item does not exist any more, callback will be skipped.");

		int msgIndex = it->first;

		if (m_isSelectModeEnabled && m_msgSelectHandler.assigned()) {
			m_msgSelectHandler(msgIndex, item->getSelectedState());
		} else if(m_msgClickHandler.assigned()) {
			m_msgClickHandler(msgIndex);
		}

	}

	void RejectMsgListView::onSelectAllClick(SelectableItem *item)
	{
		bool selectAllCheckState = m_pSelectAllItem->getSelectedState();
	...
	{
		if (m_pSelectAllItem) {
			m_pSelectAllItem->markAsSelected(chackedState);
		}
	}
	...
	{
		if (m_pSelectAllItem) {
			return m_pSelectAllItem->getSelectedState();
		}
	...
	void RejectMsgListView::setAllMsgSelected(bool chackedState)
	{
		for (auto it : m_msgItemsArray) {
			it.second->markAsSelected(chackedState);
		}
	}
	...
	bool RejectMsgListView::enableSelectMode()
	{
		...
		m_pSelectAllItem = m_pGenlist->prependItem<SelectableGroupItem>("IDS_COM_BODY_SELECT_ALL");
		m_pSelectAllItem->enableSelectMode();
		m_pSelectAllItem->setSelectHandler(SelectableItem::ItemSelectHandler::wrap<RejectMsgListView, &RejectMsgListView::onSelectAllClick>(this));

		for (MsgItemRecord itemPair : m_msgItemsArray) {
			itemPair.second->enableSelectMode();
		}
	...
	bool RejectMsgListView::disableSelectMode()
	{
		...
		for (MsgItemRecord itemPair : m_msgItemsArray) {
			itemPair.second->disableSelectMode();
		}
	...
	void RejectMsgListView::updateTitleText(int msgCount)
	{
