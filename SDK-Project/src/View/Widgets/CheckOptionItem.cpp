...
namespace Widgets {
	...
	Evas_Object *CheckOptionItem::getContent(Evas_Object *genlist, const char *part)
	{
		...
		if (strcmp(part, "elm.swallow.end") == 0) {
			...
			m_pCheckbox->setChecked(m_checkState);
			m_pCheckbox->setCheckHandler(makeNotiHandler(CheckOptionItem, onCheckChanged, this));
			m_pCheckbox->setDestroyHandler(makeNotiHandler(CheckOptionItem, onCheckboxDestroyed, this));
