...
namespace View { namespace ViewManager {
	...
	ViewType* ViewManager::pushView(ViewManager *manager, bool showAnimation)
	{
		...
		ViewType *view = View::create<ViewType>(naviItem);
	...
	bool ViewManager::isTopView(View *view)
	{
	...
	void ViewManager::onNaviItemDestroy(Elm_Object_Item *item)
	{
		RETM_IF(!item, "Callback data is not valid");

		Widgets::NaviItem *naviItem = static_cast<Widgets::NaviItem*>(elm_object_item_data_get(item));
		View *view = static_cast<View*>(naviItem->getData());
		if (view) {
