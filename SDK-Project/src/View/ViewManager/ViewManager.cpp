...
namespace View {
	...
	bool ViewManager::createWindow()
	{
		...
		if (elm_win_wm_rotation_supported_get(m_pWindow)) {
			...
			evas_object_smart_callback_add(m_pWindow, "wm,rotation,changed",
					makeEvasSmartCb(ViewManager, &ViewManager::onOrientationChanged), this);
		}
	...
	bool ViewManager::createNaviframe()
	{
		...
		eext_object_event_callback_add(m_pNaviframe, EEXT_CALLBACK_BACK,
				makeEvasSmartCb(ViewManager, &ViewManager::onHwBackKeyPressed), this);

		eext_object_event_callback_add(m_pNaviframe, EEXT_CALLBACK_MORE,
				makeEvasSmartCb(ViewManager, &ViewManager::onHwMenuKeyPressed), this);

		evas_object_smart_callback_add(m_pNaviframe, "transition,finished",
				makeEvasSmartCb(ViewManager, &ViewManager::onTransitionFinished), this);
...
/*
	...
	ViewType* ViewManager::pushView(ViewManager &manager, bool showAnimation)
	{
		...
		Evas_Object *backButton = manager.createSoftwareBackButton();
		...
		manager.m_ViewList.push_front(view);
		BaseView::ViewDestroyHandler handler = makeHandler(BaseView::ViewDestroyHandler, ViewManager, &ViewManager::onViewDestroy, &manager);
		view->setDestroyHandler(handler);

		if (showAnimation) {
			manager.isTransitionState = true;

			manager.m_pActivateJob = ecore_job_add([](void *data) {
	...
	void ViewManager::attachView(BaseView *view, bool showAnimation)
	{
		...
		BaseView::ViewDestroyHandler handler = makeHandler(BaseView::ViewDestroyHandler, ViewManager, &ViewManager::onViewDestroy, this);
		view->setDestroyHandler(handler);
	...
	Evas_Object *ViewManager::createSoftwareBackButton()
	{
		...
		if (backBtn) {
			...
			evas_object_smart_callback_add(backBtn, "clicked",
					makeEvasSmartCb(ViewManager, &ViewManager::onSoftwareBackKeyPressed), this);
		}
