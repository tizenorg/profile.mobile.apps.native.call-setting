...
namespace CallSettings { namespace Controller {
	...
	namespace {
		...
		{
			return ITEM_SUB_TEXT_COLOR_FMT.format(telNumber.getCStr());
		}
	...
	private:
		...
		void updateEnableBtnState()
		{
			if (!isInuptDataValid()) {
				m_pPopup->setButtonDisabled(POPUP_BUTTON_RIGHT, true);
				m_pEditfield->setSipReturnKeydDisabled(true);
			} else {
				m_pPopup->setButtonDisabled(POPUP_BUTTON_RIGHT, false);
				m_pEditfield->setSipReturnKeydDisabled(false);
			}
		}

		bool isInuptDataValid()
		{
	...
	public:
		...
		static Item *create(Application &app, CallFwdCondition condition, Genlist &gl,
				NotiHandler updateFinishedHandler,
				NotiHandler changeBeginHandler,
				NotiHandler changeEndHandler)
		{
		...
		void update()
		{
			if (!Model::TelRequestListener<CallFwdReqData>::isAttached()) {
				...
				DBG("Setup is already in progress, skip update request");
			}
		}
	...
	private:
		Item(Application &app, CallFwdCondition condition,
				NotiHandler updateFinishedHandler, NotiHandler changeBeginHandler, NotiHandler changeEndHandler):
			m_app(app),
			m_pItem(nullptr),
	...
	private:
		...
		bool m_isDisabled;
		NotiHandler m_updateFinihedHandler;
		NotiHandler m_changeBeginHandler;
		NotiHandler m_changeEndHandler;
	};
		...
		if (updateFlag & UF_INITIAL) {
			...
			for (int i = 0; i < VIEW_ITEM_COUNT; ++i) {
				...
				Item *obj =  Item::create(m_app, condition, gl,
						NotiHandler::wrap<CallFwController, &CallFwController::onFwdOptionReady>(this),
						NotiHandler::wrap<CallFwController, &CallFwController::onFwdOptionChangeBegin>(this),
						NotiHandler::wrap<CallFwController, &CallFwController::onFwdOptionChangeEnd>(this)
					);
	...
	void CallFwController::onFwdOptionReady()
	{
	...
	void CallFwController::onFwdOptionChangeBegin()
	{
	...
	void CallFwController::onFwdOptionChangeEnd()
	{
		DBG("onFwdOptionChangeEnd");

		bool isSyncNeeded = m_statusChangeCount > 1 ? true : false;
		m_statusChangeCount = 0;
