...
namespace CallSettings { namespace Controller {
	...
	public:
		static MsgItem *create(RejectMsgListController &listController, StringKey msgKey)
		{
			MsgItem *obj = new MsgItem(listController, msgKey);
			if (!obj->init()) {
				delete obj;
				return nullptr;
			}
	...
	private:
		MsgItem(RejectMsgListController &listController, StringKey msgKey) :
			...
			m_settingsManager(m_listController.m_app.getSettingsManager()),
			m_msgKey(msgKey),
			m_pItem(nullptr)
		{
		}
		...
		bool init()
		{
			...
			Genlist &gl = m_listController.m_pMsgListView->getGenlist();

			util::TString rejectMsgText(msgText, msgText.find("IDS_CST") == 0);
		...
		{
			m_listController.onMsgItemChecked(this);
		}

		void onClicked(WidgetItem &item)
		{
			m_listController.onMsgItemClicked(this);
		}

	private:
		RejectMsgListController &m_listController;
	...
	void RejectMsgListController::deleteSelectedMsg()
	{
		...
		int deletedMsgCount = m_selectedMsgItems.size();
		int msgTotalCount = m_msgItemsArray.size();
		int newMsgCount = msgTotalCount - deletedMsgCount;

		if (newMsgCount > 0) {
			int msgId = STRING_KEY_REJECT_MSG_ITEM_1;
			for(int i = STRING_KEY_REJECT_MSG_ITEM_1; i < STRING_KEY_REJECT_MSG_ITEM_1 + msgTotalCount; i++) {
	...
	void RejectMsgListController::updateMsgList()
	{
		...
		int msgTotalCount = m_msgItemsArray.size();
		if (newMsgCount > msgTotalCount) {
			for (int i = msgTotalCount; i < newMsgCount; i++) {
				addMessageItem(static_cast<StringKey>(STRING_KEY_REJECT_MSG_ITEM_1 + i));
			}
		} else if (newMsgCount < msgTotalCount) {
		...
		} else if (newMsgCount > 0 && msgTotalCount == 0) {
