...
#include "gui/PendingPopupContent.h"
...
namespace CallSettings { namespace Controller {
	...
	namespace {
		...
		const double FOCUS_TIMEOUT_ON_EDIT_POPUP_SHOW = 0.5;
		const double FOCUS_TIMEOUT_ON_TIMEOUT_POUP_HIDE = 0.2;
	...
	private:
		...
		bool init()
		{
			...
			if (m_callFwdReqData.condition == TELEPHONY_CF_IF_NO_REPLY) {
				...
				m_pTimeoutItem->setSubTextFormat(ITEM_SUB_TEXT_COLOR_FMT);
			...
			setEditfieldFocusTimer(FOCUS_TIMEOUT_ON_EDIT_POPUP_SHOW);
			return true;
		}
		...
		void setEditfieldFocusTimer(double timeInterval)
		{
			...
			m_pFocusTimer = ecore_timer_add(timeInterval, [](void *data) {
				...
				instance->m_pFocusTimer = nullptr;
				instance->m_pEditfield->setFocus(true);
				return ECORE_CALLBACK_CANCEL;
			}, this);
		}

		void freeEditfieldFocusTimer()
		{
			if (m_pFocusTimer) {
		...
		{
			m_pTimeoutPopup = nullptr;
			setEditfieldFocusTimer(FOCUS_TIMEOUT_ON_TIMEOUT_POUP_HIDE);
		}
	...
	private:
		...
		bool init(Genlist &gl)
		{
			...
			m_pItem->setSubTextFormat(ITEM_SUB_TEXT_COLOR_FMT);
